...
class GameState protected() {
  ...
  def startGame(context: GameController): Unit =
    error("startGame")

  /** Plays a hand. Invalid by default. */
  def playHand(context: GameController, indices: List[Int]): Int =
    error("playHand")

  /** Discards a hand. Invalid by default. */
  def discardHand(context: GameController, indices: List[Int]): List[Card] =
    error("discardHand")
  ...
  /** Returns true if this is the PreGame state. */
  def isPreGame: Boolean = false

  /** Returns true if this is the PlayerTurn state. */
  def isPlayerTurn: Boolean = false

  /** Returns true if this is the RoundEnd state. */
